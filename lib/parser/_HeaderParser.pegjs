...
select "Select" = comma? single:name_dotted more:(comma name_dotted)* comma?   { return IntermediateRepresentation.select(argumentsList(single, more)); }

filter "Filter" = comma? single:filter_item more:(comma filter_item)* comma?   { return IntermediateRepresentation.filter(argumentsList(single, more)); }
filter_item = comp

order "Order" = comma? single:order_item more:(comma order_item)* comma? { return IntermediateRepresentation.order(argumentsList(single, more)); }
