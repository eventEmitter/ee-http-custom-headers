...
select = single:name_dotted more:(comma name_dotted)* { return IntermediateRepresentation.select(argumentsList(single, more)); }

filter = single:filter_item more:(comma filter_item)* { return IntermediateRepresentation.filter(argumentsList(single, more)); }
filter_item = comp

order = single:order_item more:((comma order_item)*) { return IntermediateRepresentation.order(argumentsList(single, more)); }
